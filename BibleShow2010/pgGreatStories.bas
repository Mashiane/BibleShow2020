...
#IgnoreWarnings:12
Sub Process_Globals
	Public Name As String = "GreatStoriesCode"
	Public Title As String = "GreatStories"
...
Sub Code
	...
	cont.Hide
	Dim expgc As VMExpansionPanels = vm.CreateExpansionPanels("expgs", Me)
	expgc.SetAccordion(True)
	expgc.SetActivepanel("1")
	expgc.SetPopout(True)
	expgc.SetVisible(True)
	
	Dim recs As List = Main.GetGreatStoriesData
	...
	For Each rec As Map In recs
		...
		'
		Dim contChild As VMContainer = pgIndex.ChildContainer($"gs${childCnt}"$, sLessonScriptures, sVerses)
		expgc.AddPanel1($"pnlgs${childCnt}"$, sLessonTitle, contChild)
	Next
...
'show the page
Sub Show
	vue.setdata("heading", "Great Stories")
